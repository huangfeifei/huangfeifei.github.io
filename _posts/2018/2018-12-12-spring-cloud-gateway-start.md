...
![](https://huangfeifei.github.io/assets/images/2018/springcloud/spring-cloud-gateway.png)
...
spring:
  cloud:
    gateway:
      routes:
      - id: neo_route
        uri: https://huangfeifei.github.io
        predicates:
        - Path=/spring-cloud
```
...
上面这段配置的意思是，配置了一个 id 为 neo_route 的路由规则，当访问地址 `http://localhost:8080/spring-cloud`时会自动转发到地址：`https://huangfeifei.github.io/spring-cloud`。配置完成启动项目即可在浏览器访问进行测试，当我们访问地址`http://localhost:8080/spring-cloud` 时会展示页面展示如下：

![](https://huangfeifei.github.io/assets/images/2018/springcloud/spring-cloud-gateway1.png)
...
上面配置了一个 id 为 path_route 的路由，当访问地址`http://localhost:8080/about`时会自动转发到地址：`https://huangfeifei.github.io/about`和上面的转发效果一样，只是这里转发的是以`项目地址/about`格式的请求地址。
...
![](https://huangfeifei.github.io/assets/images/2018/springcloud/spring-cloud-gateway3.png)
